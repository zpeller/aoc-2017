...
class Particle
	attr_reader :manhattan_dst
	attr_reader :pos
	attr_reader :pid
	attr_reader :further
	@further = false

	def initialize(particle_id, pos, velocity, accel)
		@particle_id = particle_id
		@pos = pos
		@velocity = velocity
		@accel = accel
		@manhattan_dst = get_manhattan_dst()
	end

	def get_manhattan_dst
		@pos.inject(0) { |s, q| s + q.abs }
	end

	def next
		act_manh = @manhattan_dst
		(0..2).each { |q|
			@velocity[q] += @accel[q]
			@pos[q] += @velocity[q]
		}
		@manhattan_dst = get_manhattan_dst()
		@further = act_manh < @manhattan_dst
	end
	...
	def to_s
		"<##{@particle_id} P:#{@pos[0]},#{@pos[1]},#{@pos[2]} M:#{@manhattan_dst} F:#{@further? '+' : '-' }>"
	end
end

def init_particles(particles, particles_input)
	pid = 0
	particles_input.each {|pos, velocity, accel|
		particles.add(Particle.new(pid, pos, velocity, accel))
		pid+=1
	}
end

def move_particles(particles)
	particles = Marshal.load(Marshal.dump(particles))
	while particles.select { |p| p.further }.length < particles.length
		particles.each { |p| p.next }
	end

#	XXX this is just a quick hack, should make sure, that all are getting further from current min
	200.times { particles.each { |p| p.next } }
	return particles.min_by {|p| p.manhattan_dst }
end
...
#	while particles.select { |p| p.further }.length < particles.length
	200.times {
		...
		particles.each { |p| 
			p.next
			if pos_hash.key?(p.pos)
				print "Collision! #{p}<->#{pos_hash[p.pos]}\n"
				collision_list << p
				collision_list << pos_hash[p.pos]
			else
				pos_hash[p.pos] = p
			end
		}
		collision_list.each { |p| particles.delete(p) }
	}
#	end
...
particles = Set.new
init_particles(particles, input)

move_particles(particles)
print("P1: closest particle on long distance: #{move_particles(particles)}\n")
print("P2: particles after collision: #{collide_particles(particles)}\n")
