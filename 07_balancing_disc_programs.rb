...
input = (ARGV.empty? ? DATA : ARGF).each_line.map { |l|
	...
	if tokens.length > 2
		tokens2 = tokens[2].scan(/(\w+)/).freeze
		[tokens[0], tokens[1].to_i, tokens2.flatten]
	else
		[tokens[0], tokens[1].to_i, []]
	end
}.freeze

# print(input, "\n")

def init_params(prog_list, weights, programs)
	prog_list.each { |name, weight, subprogs|
		programs[name] = subprogs
		weights[name] = weight
	}
end

def find_root(programs)
	(programs.keys - programs.values.flatten)[0]
end

def find_unbalanced_branch(programs, weights, root)
	return nil, weights[root] if programs[root].empty?
	weight_list = []
	programs[root].each {|program|
		unbalanced_sub, weight = find_unbalanced_branch(programs, weights, program)
		return unbalanced_sub, weight if unbalanced_sub != nil
		weight_list << [weight, program]
	}
	return nil, weights[root] + weight_list.inject(0) {|s, x| s+x[0]}  if weight_list.map{|x,y| x}.uniq.length == 1
	print weight_list, "\n"
#	bad_weight, right_weight = weight_list.group_by{|x| x[0]}.map{|k, v| [v.length, k]}.sort
#	bad_program = weight_list.group_by{|x| x[0]}[bad_weight][0][1]
	bad_group = weight_list.group_by{|x| x[0]}.sort_by(&:length).reverse
	bad_weight, bad_program = bad_group[0].flatten.uniq
	right_weight = bad_group[1].flatten.uniq[0]
	print right_weight, "-", bad_weight, "-", bad_program, "\n"
	return bad_program, weights[bad_program] - bad_weight + right_weight 
end

weights = {}
programs = {}

init_params(input, weights, programs)
root = find_root(programs)
print("P1 root program: #{root}\n")
print("P2 bad program: #{find_unbalanced_branch(programs, weights, root)}\n")
