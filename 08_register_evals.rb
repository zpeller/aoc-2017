...
input = (ARGV.empty? ? DATA : ARGF).each_line.map { |l|
	tokens = l.strip.scan(/^(.*) (.*) (-?\d+) if (.*) (.*) (-?\d+)$/)[0].freeze
	"registers['#{tokens[0]}'] #{tokens[1]=='inc'? '+=' : '-='} #{tokens[2]} if registers['#{tokens[3]}'] #{tokens[4]} #{tokens[5]}"
}.freeze
...
def run_program(program, registers)
	local_max = 0
	program.each { |program_line|
		p program_line
		eval program_line
		local_max = [local_max, registers.values.max].max
	}
	p registers
	return registers.values.max, local_max
end

max_at_end, reg_max = run_program(input, registers)
print("P1 max register at end: #{max_at_end}\n")
print("P2 max register ever: #{reg_max}\n")
